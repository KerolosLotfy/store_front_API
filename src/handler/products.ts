...
const products_routers = async (app: express.Application) => {
    app.get("/product/:id", showPage); // show add product page
    app.get("/products/:id", showALl); // show all products
    app.post("/product/:id", uploadImage, create); // Create  New product
    app.get("/product/:id/:product_id", showOne); // show one product by id
